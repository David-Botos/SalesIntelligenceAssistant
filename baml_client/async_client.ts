...
export class BamlAsyncClient {
  ...
  async ExtractDocumentStructure(
    ...
    try {
      ...
      return raw.parsed(false) as DocumentStructure
    } catch (error) {
...
class BamlStreamClient {
  ...
  ): BamlStream<partial_types.DocumentStructure, DocumentStructure> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector ? (Array.isArray(options.collector) ? options.collector : [options.collector]) : [];
      ...
      return new BamlStream<partial_types.DocumentStructure, DocumentStructure>(
        raw,
        (a): partial_types.DocumentStructure => a,
        (a): DocumentStructure => a,
        this.ctxManager.cloneContext(),
      )
    } catch (error) {
      throw toBamlError(error);
